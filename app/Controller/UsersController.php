...
class UsersController extends AppController {
	...
	public function view($id = null) {
		...
		$user = $this -> User -> find('first', array('fields' => array('User.company', 'User.email', 'User.name', 'User.tel', 'User.fax', 'User.agent', 'User.pwd', 'User.limit_price_multi', 'User.limit_price_multi2', 'User.limit_price_multi3'), 'conditions' => array('User.id' => $id), 'contain' => array('Keyword' => array('fields' => array('Keyword.ID', 'Keyword.Keyword as keyword', 'Keyword.Url', 'Keyword.rankstart', 'Keyword.nocontract', 'Keyword.rankend', 'Keyword.Engine', 'Keyword.limit_price_group'), 'Duration' => array('conditions' => array('Duration.Flag' => 1), 'fields' => array('Duration.ID', 'Duration.KeyID', 'Duration.StartDate', 'Duration.EndDate', 'Duration.Flag'))))));
		...
		$rankhistory = $this -> User -> Keyword -> Rankhistory -> find('list', array('fields' => array('Rankhistory.KeyID', 'Rankhistory.Rank'), 'conditions' => $conds));
		...
		#$extra = $this->Extra->find('list',array('fields'=>array('Extra.ExtraType','Extra.Price'),'conditions'=>array('Extra.KeyID'=>$keyword_ids)));
 ...
 *-----------------------------------------------------------------------------------------------------------*/
	public function delete($id = null) {
		...
		if($this->Session->read('Auth.User.user.role')==1) {
			...
			$this -> Session -> setFlash(__('No Permission'), 'default', array('class' => 'error'));
			$this -> redirect($this -> referer());
 ...
 */
	...
	public function limit_price_multi() {
		...
		$this -> User -> updateAll(array('User.limit_price_multi' => $this -> request -> data['value']), array('User.id' => $this -> request -> data['pk']));
	...
	public function limit_or_not() {
		...
		$fields = array('Rankhistory.ID', 'Rankhistory.Url', 'Rankhistory.Rank', 'Rankhistory.RankDate', 'Keyword.ID', 'Keyword.UserID', 'Keyword.Keyword', 'Keyword.Engine', 'Keyword.rankend', 'Keyword.Enabled', 'Keyword.nocontract', 'Keyword.Penalty', 'Keyword.Url', 'Keyword.Strict');
