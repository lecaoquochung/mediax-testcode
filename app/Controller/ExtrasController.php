	...
	public function add($keyID, $layout = null) {
		if ($this->request->is('post')) {			
			...
			if ($this->Extra->validates()) {
				...
				$this->Session->setFlash('この順位保証は事前に登録しました。', 'default', array('class' => 'error'));
			}		
		}
		$keyword = $this->Extra->Keyword->find('first',array(
                    'fields'=>array('Keyword.ID','Keyword.Extra', 'User.id'),
                    'conditions'=>array('Keyword.ID'=>$keyID),
                    'Contain'=>array(
                        'Extra'=>array('fields'=>array('Extra.ID','Extra.ExtraType','Extra.Price'))
                    )
                ));
