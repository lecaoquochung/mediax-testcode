...
//		#10
//		$conds['Company.user_id'] = $this->Auth->user('id');
 ...
 */	
	public function admin_fileupload() {
		$this->loadModel('File');
		if($this->request->is('post')){
			// mark upload file 1: jobhunter, 2: company, 3: joboffer
			...
			if($this->File->save($this->request->data)){
				if(!empty($this->request->data['File']['filename'])){
					...
					try {
						...
						//$this->Session->setFlash( __('Error Importing') . ' ' . $this->request->data['File']['filename'] . ', ' . __('column name mismatch.')  );
						//$this->redirect( array('action'=>'import') );
					}
					$new_records_count = $this->Company->find( 'count' ) - $records_count;				
					//$csv = $this->Csv->import(Configure::read('FOLDER_UPLOAD_CSV').'/'.$this->request->data['File']['filename'],Configure::read('HEADER_CSV'));
				}			
			}			
		}		
		// Condition: mark & user_id
