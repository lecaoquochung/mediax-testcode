...
class Rankhistory extends AppModel {
		...
		public function header_csv_all_keyword($headers, $mapHeader) {
            $data_headers = array();
            foreach ($headers as $key => $header) {
                if (isset($mapHeader[$header]) && !empty($mapHeader[$header])) {
                    if($header=='Rankhistory.Rank'){
		 ...
		 public function row_csv_all_keyword($values) {
            $data_values = array();
            foreach ($values as $key => $value) {
                if($key=='Rankhistory.Rank'){
                	if (strpos($value, '/') !== false) {
						...
						$engines = $value;
	                    $data_values[$key.'.Google'] = trim($engines);
						$data_values[$key.'.Yahoo'] = 0;
					}
                }else{
                    $data_values[$key] = $value;
                }                        
