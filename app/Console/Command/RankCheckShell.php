...
class RankCheckShell extends Shell {
	...
	public function main() {
		...
		foreach ($keywords as $keyword) {
			...
			if ($engine == 3) {
				...
				$rank = $this -> Rank -> keyWordRank('google_jp', $domain, $keyword['Keyword']['Keyword']);
			} elseif ($engine == 2) {
				$rank = $this -> Rank -> keyWordRank('yahoo_jp', $domain, $keyword['Keyword']['Keyword']);
			} elseif ($engine == 6) {
			...
			if (isset($data_rankhistory['Rankhistory']['Rank']) && strpos($data_rankhistory['Rankhistory']['Rank'], '/')) {
				...
				$rank_old[0] = explode('/', $data_rankhistory['Rankhistory']['Rank']);
				$rank_old[1] = 0;
			} else {
				$rank_old[0] = 0;
				$rank_old[1] = 0;
