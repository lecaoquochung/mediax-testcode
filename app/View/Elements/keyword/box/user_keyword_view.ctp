...
<div class="box admin_students_view span12">
	<div class="navbar">
		<div class="navbar-inner">
			<h3 class="brand"><?php echo __('Keyword Rankhistory'); ?></h3>
		</div>
	</div>
	<div class="section">
		...
		<div class="company-info">
			<p class="lead">
				...
				<?php echo $this->Html->link($this->Text->truncate($keyword['Keyword']['Url'], 35), $keyword['Keyword']['Url'], array('target' => '_blank')); ?>
			</p>
		</div>
<!-- csv -->
		<div class="csv-download">
			<!-- load rank -->
			<?php echo $this->Session->read('Auth.User.user.role')==2 ? $this->Html->link(__('Load Rank'), 'javascript:void(0)',array('class'=>'loadRank btn btn-danger','KeyID'=>$keyword['Keyword']['ID'])) .$this->Html->image('loading.gif',array('class'=>'loading','style'=>'display:none')):''; ?>
			<?php echo $this->Html->link(__('CSV'), array('controller' => 'rankhistories', 'action' => 'csv_by_keyword',$keyword['Keyword']['ID']), array('class' => 'btn btn-success')); ?>
		</div>
...
<!-- rank data history -->
		<div class="form-group" >
			<div class="date-range">
				<span class="fleft">
					...
					?>
					<?php echo '&nbsp'; ?>
					<?php echo $this->Form->submit('実行',array('class'=>'btn btn-info', 'div' => FALSE)); ?>
					<?php echo $this->Form->end() ?>
				</span>
			
				<span class="fright">
					<?php if(!isset($this->request->params['named']['page'])) : ?>
					<span class="label label-success"><?php echo __('Choose date range'); ?></span>
					...
					?>
					<?php echo $this->Form->button(__('Choose'), array('class'=>'btn btn-success', 'div'=>FALSE)); ?>
					<?php echo $this->Form->end(); ?>
					<?php endif ?>
				</span>
			</div>
		...
		<div class="common-button">
			...
			<?php echo $this->Html->link(__('1~10'), array('controller' => 'keywords', 'action' => 'view',$keyword['Keyword']['ID'], 10), array('class' => 'btn btn-info')); ?>
			<?php echo $this->Html->link(__('キーワード特別条件'), array('controller' => 'extras', 'action' => 'add', $keyword['Keyword']['ID']), array('class' => 'btn btn-warning')); ?>
		</div>
		<!--Table-->
		<table class="table tableX">
			<tr>
				<th class="tbl3"><?php echo __('RankDate'); ?> <span class="label"><?php echo count($data_rankhistories).'日' ?></span></th>
				<th class="tbl3">
					<?php 
						global $list_engine;
